...
| [跨层契约](./contracts.md) | CLI、OCR JSON、单页工作区、阶段图、16:9 与 PPTX 边界、双人工点闸门与瞬态阶段失效、多代资产的「当前产物」判据、**来源规则派生状态的重判点与消费端**、原生二进制契约边界、独立契约文件的版本轴、**模型可提案不可直接落盘** | M1 部分已验证；M4 链路契约已由端到端走查验证；M5 四节由真实 deck、真实 PDF 与阶段三集成走查验证；末节「模型可提案不可直接落盘」是 **M6 规划期决策，实现尚未落地** |
