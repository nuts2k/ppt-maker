...
`nuts2k` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/nuts2k/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/nuts2k/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python3 ./.trellis/scripts/task.py list --assignee nuts2k` to
...
python3 ./.trellis/scripts/task.py archive 00-join-nuts2k
```
