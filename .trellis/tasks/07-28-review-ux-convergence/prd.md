...
- [x] AC1 单页视图中，阶段轨道上的任何点位点击都不触发重跑；轨道仍完整显示 9 个阶段的
      状态色与阶段名。
- [x] AC2 `SlideToolbar` 上不存在「从阶段重跑」菜单；单页可用的执行动作恰为 R1.5 所列四项。
- [x] AC3 某阶段失败时，`StageRail` 错误条上出现单个「重跑失败阶段」按钮，点击后该阶段
      及下游转 stale 并从该阶段重跑（不复现「点了没反应」）。
- [x] AC4 完成一页验收后，无需任何额外点击，轨道即显示 9/9 全部完成，且
      `stages/report/report.json` 已更新。
- [x] AC5 批量续跑时，已验收完成的页不再被重新拉进队列。
- [x] AC6 report 补跑失败时，界面仍显示验收成功，活动日志中有一条对应的失败记录。
- [x] AC7 在文本复核列表中对任意一项按 ⌥1/⌥2 改分类，该项在列表中的**位置不变**、
      仍处于当前筛选结果内，其标签同步更新。
- [x] AC8 筛选条各档的计数与实际条目数一致；筛选「未复核」时的条目集合等于
      `reviewStatus === "unreviewed"` 的块集合。
- [x] AC9 「跳到下一个未复核项」快捷键能从任意位置跳到下一个未复核项；已到末项时的行为
      明确且不静默失败。
- [x] AC10 标记某项已复核后，该项仍显示在原位置（淡化打勾），列表不发生重排。
- [x] AC11 从最终确认页点「回到文本复核」进入时，筛选默认为「全部」，且**没有任何块的
      `reviewStatus` 被改变**（对比操作前后的 `text-blocks.json`）。
- [x] AC12 人工编辑过的块（`updatedAt` 非空）在列表项上有可见的「已修改」标记。
- [x] AC13 在全部阶段 completed 的页上改块分类并保存后，manifest 中 `mask` 及其下游转 stale；
      点「运行此页」能把改动带到 clean/pptx 产物。
- [x] AC14 只修改块文本（不影响 mask 投影）并保存后，`mask` 保持 completed、`pptx` 转 stale，
      不触发 clean 的付费调用。
- [x] AC15 保存后阶段轨道立即反映 stale 状态，不出现「磁盘 stale、轨道一片绿」。
- [x] AC16 对 clean 跑过两次的页生成 report，`clean_record` 指标取自最后一次成功的 attempt
      （真实工作区 page-01 应读出 `outsideMaskDiff = 0.0439`）。
- [x] AC17 `pnpm format:check`、`pnpm typecheck`、`pnpm test` 全绿
