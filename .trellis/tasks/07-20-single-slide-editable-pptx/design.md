...
- `analyze` 和 `clean` 必须带显式上传确认；不得由低置信度或 `run` 静默触发。
...
<workspace>/
  ...
  stages/
    ocr/result.json
    analyze/result.json
    review/text-blocks.json # 唯一主要人工编辑入口
    mask/mask.png
    mask/preview.png
    mask/overlay.png
    mask/record.json
...
init
 ├─→ ocr ─┐
 └────────→ analyze ─→ review ─→ mask ─→ clean ─→ accept-clean ─→ pptx ─→ accept-pptx ─→ report
```

- `analyze` 同时读取源图、OCR 候选和可选原始文案。
- `review` 合并所有候选，但人工确认值独立保存，不被再次分析静默覆盖。
- 修改 reference 使 analyze 及下游失效，不使 OCR 失效。
...
输入为完整源图、OCR 结果和可选参考文案。严格输出候选文字块、漏字提示、旋转、艺术字标记、分类、样式估计、前景颜色候选和解释性风险标记。它不得直接修改 review 文件中的人工确认值。
...
- OCR 默认离线；只有 `analyze --confirm-upload` 和 `clean --confirm-upload` 上传图片。
...
- 云端视觉结构化输出不稳定：保留离线 OCR 和人工 review，修正提示词/Schema，不允许静默解析自由文本。
