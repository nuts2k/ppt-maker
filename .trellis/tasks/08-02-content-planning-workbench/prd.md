...
- **A1**：零页 scratchpad 从一句构思经两轮 `gpt-5.6-luna` 提问收敛五维度，生成 6 条完整
  初稿并整体接受；从中建立最小一页，真实生成源图、复核、clean、原生 PPTX，最终
  `deck export --strict` 成功输出 1 页原生 PPTX，全程未手写 JSON。
- **A2**：`~/test/wt4-spec-2026-08-02` 副本真实单条目改稿并接受，只有 page-01 新增漂移，
  page-02～04 字节不变；全 deck 作用域由受控 Provider 集成测试覆盖。
- **A5**：真实初稿与改单均写出 `origin="proposal"`，`conversationRef` 与会话 proposal message
  双向可追溯；把完成 deck 的整个 `planning/` 移出后，`status`、`run`、`export --strict` 仍成功。
- **A9**：最终 89 个测试文件、948 项通过（core 156、CLI 265、desktop 527）。

限制如实保留：首次 clean 有标题残留，走“重做底图”后第二次通过人工图像检查；网关两次返回
1672×941 而非声明的 2048×1152，自动记录为尺寸异常但保持 16:9；PowerPoint 已发起打开，机器
锁屏导致无法取得应用内截图，PPTX 六项自动检查均通过。

## 集成验收结论（父任务，2026-08-04）

三个子任务全部完工归档，父级逐条核对 A1–A9，**九条全部满足，无遗留缺口**。
本轮父级验收不再新增付费调用——A1 / A2 / A4 / A8 的真实出图证据已由子任务② ③ 在各自
走查中产生并留档，重复跑只是重复花钱。

| 条目 | 结论 | 证据 |
|---|---|---|
| A1 一句构思 → 规格 → `--strict` PPTX，全程不写 JSON | 满足 | 对话半条见 ③ A1（零页 scratchpad 两轮 `gpt-5.6-luna` 提问 → 6 条初稿 → 建页 → strict 导出）；非对话半条见 ② E1（空 deck 手工写规格走通同一终点） |
| A2 逐页对话式改稿、M5 如实报漂移 | 满足 | ③ A2：`wt4-spec-2026-08-02` 副本单条目改稿后只 page-01 新增漂移，page-02～04 字节不变；全 deck 作用域由受控 Provider 集成测试覆盖 |
| A3 打开已有 deck 完成全部编辑，含 `revisionNotes` 删除 | 满足 | ② 第 1 条验收：全程不碰 JSON 完成所有编辑，补上 M5「只能追加、看不见已累积条目」的缺口 |
| A4 改 `style` 后清单列全，批量重生成未勾选页字节不变 | 满足 | ② 第 3 条验收（做过变异验证）；① A①-2 在 CLI 层以页目录递归内容哈希比对 |
| A5 变更日志可回看可回滚，删除后功能照常 | 满足 | ① A①-3 / A①-9（连做两次回滚，历史只增不减）；② 第 5 条（界面回看与回滚，`historyWritten === false` 出声）；③ A9（移走整个 `planning/` 后 `status` / `run` / `export --strict` 仍成功） |
| A6 零迁移，旧格式 deck 不报错不被改写 | 满足 | ① A①-4（`ppttest-2026-07-25` 只读命令跑完目录内容零变化、不产生 `planning/`）；② 第 6 条（工作台打开同一 deck） |
| A7 生成侧契约未变 | 满足 | 本次父级复核：`git diff 51a1823^..HEAD -- packages/core/src/content-spec-contracts.ts packages/core/src/constants.ts apps/cli/src/providers/page-generation.ts` 输出为空，M6 全程逐字未变 |
| A8 付费门槛写明次数与不可撤销 | 满足 | ② 第 4 条；③ A10（本轮新增的文本模型调用不绕过既有 provider 错误与追踪纪律） |
| A9 测试基线不倒退 | 满足并大幅提升 | 774 → **948**（core 156 / desktop 527 / CLI 265，89 个测试文件）。本次父级复核重跑 `format:check` / `typecheck` / `test` / `build` 四关全绿，工作区干净 |

补充结论：

- **写入路径唯一性**（① A①-8 的父级复检）：`rg -n "writeDeckContentSpec" apps packages --glob '!**/test/**'`
  只剩定义与 `spec-edit.ts` 内的唯一调用，全仓无第二条写 deck 内 `content-spec.json` 的生产路径。
- **D5 放宽已回写规范**：`.trellis/spec/backend/contracts.md:904` 起〈模型可提案、不可直接落盘〉
  与 `:1071`〈规格写入唯一入口与变更日志〉均已落库；三个新错误码登记于
  `.trellis/spec/backend/error-handling.md:31-33`。风险表「D5 放宽未回写规范」一条已消解。
- **A9 原文的基线数字 774 已过时**：子任务① 完工后实测 854，②后 877，③后 948。以 948 为新基线。
- 如实保留的既有限制（非本里程碑缺陷）：第三方网关不返回 `x-request-id`，变更日志中
  `requestId` 记 `null` 不伪造；图像网关返回 1672×941 而非声明尺寸，按既有口径记尺寸异常并保持 16:9。
