...
- [ ] 1.1 更新 `ROADMAP.md` M6 小节：状态改「执行中」，补记 D1–D9 九条决策，
      订正「M6 的策划界面到底能改哪些东西」一节——D8 已定 ①⑤ 与调用参数不开放。
- [ ] 1.2 订正 ROADMAP / 契约注释里「`content-spec.json` 有自己的版本轴」的说法：
      `SCHEMA_VERSION` 是全仓共用常量（`packages/core/src/constants.ts:1`），
      各契约写死 `z.literal`，该表述在实现上不成立，留着会误导后来者做出全仓迁移级的改动。
- [ ] 1.3 把 **D5 对 D7 的放宽**写进 `.trellis/spec/`：
      「模型可提案、不可直接落盘」，及放宽后仍成立的三条（design §6）。
      不写这条，后来者按 M5 父任务 `prd.md:44` 原文会判定本里程碑违规。
- [ ] 1.4 创建三个子任务并写入各自 `prd.md` 的目标与边界引用（见下）。

阶段一不动任何 `apps/` / `packages/` 下的代码。

## 阶段二：创建并推进子任务

```bash
python3 ./.trellis/scripts/task.py create "<标题>" --slug <slug> --parent 08-02-content-planning-workbench
```

| 顺序 | slug | 交付物 | 依赖 |
|---|---|---|---|
| ① | `spec-edit-and-history` | core/CLI：统一写入入口 `applySpecChange`、变更日志读写与回滚、批量重生成命令、零页 deck 边界 | 无 |
| ② | `planning-view` | 桌面端：新增 `AppView` `planning`、条目列表与逐字段编辑器、已过时页清单与勾选确认 | ① |
| ③ | `planning-conversation` | 策划提问 provider、会话落盘、改稿提案与 diff 确认、背景材料输入 | ①② |
...
| A7 契约未变 | `git diff` 确认 `content-spec-contracts.ts` / `page-generation.ts` / `constants.ts` 无改动 | — |
