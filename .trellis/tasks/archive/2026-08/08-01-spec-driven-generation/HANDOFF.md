...
**6.（规划后补）`deck generate` 必须支持往已有 deck 追加，且对账只覆盖 generated 页。**
父任务 A2 要求交错混合三种来源，而 `addSlideToDeck` 只能追加末尾且 `page-NN` 分配后
不重排，因此混合 deck 靠**按页序依次调用不同来源的命令**实现——只支持「建新 deck」
会让 A2 无法实现。命令形态与子任务② 的 `deck extract` 同构（deck 不存在则创建）。
对账若不限定在 `source.kind === "generated"`，往混合 deck 跑一次 `deck generate`
就会把所有导入页与抽取页报成「失联」。这条是规划②时发现并回写的，验收见 C14。
