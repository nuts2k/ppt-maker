...
import { isRunStage, type RunStage } from "../../../shared/stages.js";

/**
 * 单页阶段轨道（design.md 3.3 StageRail）—— 画布上方常驻，不再藏进侧边栏标签页。
 *
 * V1 的关键缺陷之一是"错误只在侧边栏 Pipeline 标签短暂显示"。这里把 9 阶段轨道
 * 与失败详情提到常驻位置，失败时错误条直接展开 `code: message`
 * （耐久层来自 manifest attempts，重启后依然可见）。
 *
          ...
          <div className="flex items-center gap-3">
            ...
            {rerunStage !== null && (
              <button
                ...
                className="shrink-0 rounded-xs border border-on-primary/40 px-2 py-0.5 text-sm font-medium text-on-primary transition active:border-on-primary disabled:opacity-50"
              >
                重跑失败阶段
              </button>
            )}
