...
export function ReviewPage(): React.JSX.Element {
   ...
   * 视图默认落点：**有产物就先给结果**。
   *
   * 判据取「确认页可达」（`finalGate !== null`，即 pptx 已产出），而不再排除已验收页。
   * 此前的判据是「待确认」，理由记作"进已验收页多半是想看复核内容"——实际相反：
   * pptx 跑完意味着文本复核门早已通过，此时回头改文字是小概率事件，而复核界面
   * （480px 列表 + 标注画布 + 快捷键条）恰恰是全应用最重的一屏。常态需求是看一眼
   * 最终效果，把它藏在一次手动切档后面，等于每次进页都要多点一下。
   *
   * 反过来不成立的情形由 `finalGate === null` 兜住：产物未生成或被作废（「重做底图」
   * 「回到文本复核」都会失效 pptx）时确认页无内容可看，回落文本复核。
   *
   * 签名含 slideId 与 source：切页按新页重算，「重做底图 → 重新停在最终确认」也会
   * 再次进入；用户在页内手动切回文本复核后签名不变，effect 不重跑，不会被拉回去。
   */
  const gateSignature =
  ...
  useEffect(() => {
    if (gateSignature === null) {
      // 产物未生成或已作废：确认页此刻没有东西可看，不把用户留在空屏上
