...
export const CLEAN_PLATE_PROMPT_VERSION = "m1-clean-plate-v5";
...
export const CLEAN_PLATE_PROMPT = [
  ...
  "严格边界约束：只修改 mask 透明区域内的像素。即使透明区域与周围的装饰图案、词云、图表、插画在视觉上属于同一个卡片或容器，也绝对不能改动透明区域以外的任何内容。不透明区域的每一个像素都必须与原图完全一致。",
