...
- 绝对禁止过度工程化或过度防御性编程。大多数项目只对关键路径和核心主流程设置测试与约束。
- 优先保证主要路径不出错，并让未直接覆盖的错误或生产环境中的报错可追溯；出现问题后根据错误信息定位并修复，不追求在测试阶段发现所有潜在问题。
- 根据项目规模合理规划测试。大型或高度复杂的项目可以适度提高覆盖范围；对这类项目进行全面测试不视为过度工程化。
- 禁止主动、反复审查已经检查过或显然经过多次验证的主力部分和旧代码，正在修复相关 Bug 或用户明确要求时除外。
- 上下文压缩后，不要在下一轮重新检查此前工作是否正确；只需探查到足以确认当前进度和待办事项的程度。
